...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  else { // 200에서 최대 100,300에서 최소
    if (dist_raw <= 200){
      red = 255 * (200 - dist_raw) / 100;
    }
    else{
      red = 255 * (dist_raw - 200) / 100;
    }
    analogWrite(PIN_LED, red);
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading == 0) reading = last_reading; //리딩값이 0이면 이전값 사용
  last_reading = reading; //리딩값 저장
  return reading;
