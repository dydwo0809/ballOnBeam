...
#define _DIST_ALPHA 0.35 //ema의 알파값

#define _DUTY_MIN 553 // servo full clockwise position (0 degtree)
...
// get a distance reading from the USS
  ...
  if (dist_ema == 0){  // 맨 처음 값이 alpha * dist_raw 되는거 방지
    dist_ema = dist_raw;
  }
  else{
...
if(dist_ema < 180.0) { // 18~36cm 사이에서 거리에 비례해서 모터 돌아감
     myservo.writeMicroseconds(_DUTY_MIN);
     analogWrite(PIN_LED, 255);
  }
  else if(dist_ema < 360.0){
     myservo.writeMicroseconds((_DUTY_MIN) + (((dist_ema) - 180.0) / 180.0) * (_DUTY_MAX - _DUTY_MIN));
     analogWrite(PIN_LED, 0);  // LED 켜짐
  }
  else {
