...
#define _SERVO_SPEED 670  //[2959] 서보의 속도 설정

// Event periods
#define _INTERVAL_DIST 20  //[2959] 센서의 거리측정 인터벌값
#define _INTERVAL_SERVO 20 //[2967] 서보 INTERVAL값 설정
#define _INTERVAL_SERIAL 300  //[2959] 시리얼 모니터/플로터의 인터벌값 설정

// PID parameters
#define _KP 0.33 // [2957] 비례 제어 값
                // [2961] 비례이득
...
////////////////////
  ...
  // duty_target = f(duty_neutral, control)
    duty_target = _DUTY_NEU + _KP * (_DIST_TARGET - dist_ema); 
  ...
  if(event_servo) {
    ...
    if(duty_target > duty_curr) {  // [2964] 현재 서보 각도 읽기
      duty_curr += 1.2 * duty_chg_per_interval; // [2961] duty_curr은 주기마다 duty_chg_per_interval만큼 증가
