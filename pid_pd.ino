...
#define _DUTY_NEU 1385 // [2952] 서보의 중간 각도값
...
////////////////////
  ...
  // PID control logic
    error_curr = pterm;                      // [2959] 직전 pterm
    pterm = _DIST_TARGET - dist_ema;         // [2959] PID의 P
    
    if (dterm * (pterm - error_curr) < 0){
      sign = 0;
    }
    else{
      sign = 1;
    }
    dterm = sign * (pterm - error_curr);              // PID의 D
    
    control = _KP * pterm + _KD * dterm;     // [2959] 중간에서 얼마나 움직일 것인가

  // duty_target = f(duty_neutral, control)
    if (_DIST_TARGET > dist_ema){
      duty_target = _DUTY_NEU + 1.08 * control; 
    }
    else{
  ...
  if(event_serial) {
    ...
    Serial.print(",pterm:");
    Serial.print(map(pterm * _KP,-1000,1000,510,610));   
    Serial.print(",dterm:");
    Serial.print(map(dterm * _KD,-1000,1000,510,610));    
